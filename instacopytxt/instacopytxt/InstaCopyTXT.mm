...
static NSBundle* getBundle() {
    return [NSBundle bundleWithPath:@"/Library/PreferenceBundles/InstaCopyTXT.bundle"];
}
...
#define tweak_defaults @"/User/Library/Preferences/com.imokhles.InstaCopyTXT.plist"
...
@interface InstaCopyTXTBannerCell : PSTableCell {
    UIImageView *_imageView;
}
@end

@interface InstaCopyTXTListController: PSListController {
}
@end

@implementation InstaCopyTXTListController
- (id)specifiers {
	if(_specifiers == nil) {
		_specifiers = [[self loadSpecifiersFromPlistName:@"InstaCopyTXT" target:self] retain];
	}
	return _specifiers;
}
-(void)loadView {
...
-(void)shareTapped:(UIBarButtonItem *)sender {
    ...
    NSString *language = [[[NSBundle mainBundle] preferredLocalizations] objectAtIndex:0];
    if ([language isEqualToString:@"ar"]) {
        text = @"استخدم اداة #InstaCopyTXT بواسطة @iMokhles لنسخ النصوص من تطبيق الانتسجرام بشكل سريع";
    } else {
        text = @"I like #InstaCopyTXT by @iMokhles to copy Instagram app posts/comments texts.";
    }    
...
- (NSString *)versionValue:(PSSpecifier *)specifier {
    ...
    [task setArguments:[NSArray arrayWithObjects: @"-c", @"dpkg -s com.imokhles.InstaCopyTXT | grep 'Version'", nil]];
...
@implementation InstaCopyTXTBannerCell
- (id)initWithSpecifier:(PSSpecifier *)specifier
{
    ...
    if (self) {
        ...
        if (IS_IPHONE) {
            _imageView = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:[getBundle() pathForResource:@"InstaCopyTXTHeader" ofType:@"png"]]];
