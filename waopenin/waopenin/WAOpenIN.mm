...
static NSBundle* getBundle() {
    return [NSBundle bundleWithPath:@"/Library/PreferenceBundles/WAOpenIN.bundle"];
}
...
#define tweak_defaults @"/User/Library/Preferences/com.imokhles.WAOpenIN.plist"
...
@interface WAOpenINBannerCell : PSTableCell {
    UIImageView *_imageView;
}
@end

@interface WAOpenINListController: PSListController {
}
@end

@implementation WAOpenINListController
- (id)specifiers {
	if(_specifiers == nil) {
		_specifiers = [[self loadSpecifiersFromPlistName:@"WAOpenIN" target:self] retain];
	}
	return _specifiers;
}
-(void)loadView {
...
-(void)shareTapped:(UIBarButtonItem *)sender {
    ...
    NSString *language = [[[NSBundle mainBundle] preferredLocalizations] objectAtIndex:0];
    if ([language isEqualToString:@"ar"]) {
        text = @"استخدم اداة #WAOpenIN بواسطة @iMokhles لمشاركة صور الواتس اب للتطبيقات الاخري";
    } else {
        text = @"I like #WAOpenIN by @iMokhles to share WhatsApp images with other apps.";
    }    
...
- (NSString *)versionValue:(PSSpecifier *)specifier {
    ...
    [task setArguments:[NSArray arrayWithObjects: @"-c", @"dpkg -s com.imokhles.WAOpenIN | grep 'Version'", nil]];
...
@implementation WAOpenINBannerCell
- (id)initWithSpecifier:(PSSpecifier *)specifier
{
    ...
    if (self) {
        ...
        if (IS_IPHONE) {
            _imageView = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:[getBundle() pathForResource:@"WAOpenINHeader" ofType:@"png"]]];
