...
static NSBundle* getBundle() {
    return [NSBundle bundleWithPath:@"/Library/PreferenceBundles/InstaOpenIN.bundle"];
}
...
#define tweak_defaults @"/User/Library/Preferences/com.imokhles.InstaOpenIN.plist"
...
@interface InstaOpenINBannerCell : PSTableCell {
    UIImageView *_imageView;
}
@end
@interface InstaOpenINListController: PSListController {
}
@end

@implementation InstaOpenINListController
- (id)specifiers {
	if(_specifiers == nil) {
		_specifiers = [[self loadSpecifiersFromPlistName:@"InstaOpenIN" target:self] retain];
	}
	return _specifiers;
}
-(void)loadView {
...
-(void)shareTapped:(UIBarButtonItem *)sender {
    ...
    NSString *language = [[[NSBundle mainBundle] preferredLocalizations] objectAtIndex:0];
    if ([language isEqualToString:@"ar"]) {
        text = @"استخدم اداة #InstaOpenIN بواسطة @iMokhles لفتح الصور و الفيديوهات من تطبيق الانستجرام لاي تطبيق اخر";
    } else {
        text = @"I like #InstaOpenIN by @iMokhles to open instagram's images and videos within other apps";
    }    
...
- (NSString *)versionValue:(PSSpecifier *)specifier {
    ...
    [task setArguments:[NSArray arrayWithObjects: @"-c", @"dpkg -s com.imokhles.InstaOpenIN | grep 'Version'", nil]];
...
@implementation InstaOpenINBannerCell
- (id)initWithSpecifier:(PSSpecifier *)specifier
{
    ...
    if (self) {
        ...
        if (IS_IPHONE) {
            _imageView = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:[getBundle() pathForResource:@"InstaOpenINHeader" ofType:@"png"]]];
