...
static NSBundle* getBundle() {
    return [NSBundle bundleWithPath:@"/Library/PreferenceBundles/FBCopyTXT.bundle"];
}
...
#define tweak_defaults @"/User/Library/Preferences/com.imokhles.FBCopyTXT.plist"
...
@interface FBCopyTXTBannerCell : PSTableCell {
    UIImageView *_imageView;
}
@end

@interface FBCopyTXTListController: PSListController {
}
@end

@implementation FBCopyTXTListController
- (id)specifiers {
	if(_specifiers == nil) {
		_specifiers = [[self loadSpecifiersFromPlistName:@"FBCopyTXT" target:self] retain];
	}
	return _specifiers;
}
-(void)loadView {
...
-(void)shareTapped:(UIBarButtonItem *)sender {
    ...
    NSString *language = [[[NSBundle mainBundle] preferredLocalizations] objectAtIndex:0];
    if ([language isEqualToString:@"ar"]) {
        text = @"استخدم اداة #FBCopyTXT بواسطة @iMokhles لنسخ كلام البوستات في الفيس بوك بكل سهولة";
    } else {
        text = @"I like #FBCopyTXT by @iMokhles to copy facebook app posts.";
    }    
...
- (NSString *)versionValue:(PSSpecifier *)specifier {
    ...
    [task setArguments:[NSArray arrayWithObjects: @"-c", @"dpkg -s com.imokhles.FBCopyTXT | grep 'Version'", nil]];
...
@implementation FBCopyTXTBannerCell
- (id)initWithSpecifier:(PSSpecifier *)specifier
{
    ...
    if (self) {
        ...
        if (IS_IPHONE) {
            _imageView = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:[getBundle() pathForResource:@"FBCopyTXTHeader" ofType:@"png"]]];
