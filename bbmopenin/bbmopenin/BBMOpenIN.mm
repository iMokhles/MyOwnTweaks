...
static NSBundle* getBundle() {
    return [NSBundle bundleWithPath:@"/Library/PreferenceBundles/BBMOpenIN.bundle"];
}
...
#define tweak_defaults @"/User/Library/Preferences/com.imokhles.BBMOpenIN.plist"
...
@interface BBMOpenINBannerCell : PSTableCell {
    UIImageView *_imageView;
}
@end

@interface BBMOpenINListController: PSListController {
}
@end

@implementation BBMOpenINListController
- (id)specifiers {
	if(_specifiers == nil) {
		_specifiers = [[self loadSpecifiersFromPlistName:@"BBMOpenIN" target:self] retain];
	}
	return _specifiers;
}
-(void)loadView {
...
-(void)shareTapped:(UIBarButtonItem *)sender {
    ...
    NSString *language = [[[NSBundle mainBundle] preferredLocalizations] objectAtIndex:0];
    if ([language isEqualToString:@"ar"]) {
        text = @"استخدم اداة #BBMOpenIN بواسطة @iMokhles لمشاركة صور البي بي ام للتطبيقات الاخري";
    } else {
        text = @"I like #BBMOpenIN by @iMokhles to share BBM images with other apps.";
    }    
...
- (NSString *)versionValue:(PSSpecifier *)specifier {
    ...
    [task setArguments:[NSArray arrayWithObjects: @"-c", @"dpkg -s com.imokhles.BBMOpenIN | grep 'Version'", nil]];
...
@implementation BBMOpenINBannerCell
- (id)initWithSpecifier:(PSSpecifier *)specifier
{
    ...
    if (self) {
        ...
        if (IS_IPHONE) {
            _imageView = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:[getBundle() pathForResource:@"BBMOpenINHeader" ofType:@"png"]]];
